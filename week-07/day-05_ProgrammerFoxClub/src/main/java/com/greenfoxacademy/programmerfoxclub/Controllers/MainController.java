...
//        model.addAttribute("foxes", foxService.getFoxList()); //NE HASZNALJ LISTAT, MERT BELEBUKSZ ;)
    ...
    @PostMapping("/nutritionStore/add")     //here  just make a plus /add place (just for reference, I will never go there actually) / this way it will not crash with the getmappingg for the same page
    public String addNutrition(Model model, @RequestParam(value = "name", required=false) String name, @ModelAttribute("food") String food, @ModelAttribute("drink") String drink, RedirectAttributes attributes) {
        Optional<Fox> optionalFox = foxService.findFox(name);
    ...
    public String addTricks(Model model, @RequestParam(value = "name", required=false) String name, @ModelAttribute("trick") String trick, RedirectAttributes attributes) {
        Optional<Fox> optionalFox = foxService.findFox(name);
